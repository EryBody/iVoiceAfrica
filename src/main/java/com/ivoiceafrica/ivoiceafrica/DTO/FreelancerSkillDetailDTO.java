package com.ivoiceafrica.ivoiceafrica.DTO;

public class FreelancerSkillDetailDTO {

	private String translationServiceType;
	private String translationExperience;
	private String translationIndustries;
	private String translationLanguages;
	private String translationLanguage1;
	private String translationUpload1;
	private String translationLanguage2;
	private String translationUpload2;
	private String translationLanguage3;
	private String translationUpload3;
	private String translationLanguage4;
	private String translationUpload4;
	private String translationLanguage5;
	private String translationUpload5;
	private String translationPortfolioLink;
	private String translationPerWordMin;
	private String translationPerWordMax;
	private String interServiceType;
	private String iexperience;
	private String interpretationLanguages;
	
	private String interLanguage1;
	private String interUpload1;
	
	private String interLanguage2;
	private String interUpload2;
	
	private String interLanguage3;
	private String interUpload3;
	
	private String interLanguage4;
	private String interUpload4;
	
	private String interLanguage5;
	private String interUpload5;
	private String interPortfolioLink;
	private String interPerWordMin;
	private String interPerWordMax;
	
	
	private String vcServiceType;
	private String vcExperience;
	private String vcIndustries;
	private String vcLanguages;
	
	private String vcLanguage1;
	private String voiceDesc1;
	private String vcUpload1;
	
	private String vcLanguage2;
	private String voiceDesc2;
	private String vcUpload2;
	
	private String vcLanguage3;
	private String voiceDesc3;
	private String vcUpload3;
	
	private String vcLanguage4;
	private String voiceDesc4;
	private String vcUpload4;
	
	private String vcLanguage5;
	private String voiceDesc5;
	private String vcUpload5;
	
	private String vcPortfolioLink;
	
	private String vcPerMinuteMin;
	private String vcPerMinuteMax;

	private String vcPerHourMin;
	private String vcPerHourMax;
	
	private String vcPerWordMin;
	private String vcPerWordMax;
	
	
	
	
	private String scribingServiceType;
	private String scribingExperience;
	private String scribingIndustries;
	private String transcribeLanguages;
	
	private String transcribeLanguage1;
	private String transcribeUpload1;
	
	private String transcribeLanguage2;
	private String transcribeUpload2;
	
	private String transcribeLanguage3;
	private String transcribeUpload3;
	
	private String transcribeLanguage4;
	private String transcribeUpload4;
	
	private String scribingPortfolioLink;
	private String scribingPerWordMin;
	
	private String scribingPerWordMax;

	public String getTranslationServiceType() {
		return translationServiceType;
	}

	public void setTranslationServiceType(String translationServiceType) {
		this.translationServiceType = translationServiceType;
	}

	public String getTranslationExperience() {
		return translationExperience;
	}

	public void setTranslationExperience(String translationExperience) {
		this.translationExperience = translationExperience;
	}

	public String getTranslationIndustries() {
		return translationIndustries;
	}

	public void setTranslationIndustries(String translationIndustries) {
		this.translationIndustries = translationIndustries;
	}

	public String getTranslationLanguages() {
		return translationLanguages;
	}

	public void setTranslationLanguages(String translationLanguages) {
		this.translationLanguages = translationLanguages;
	}

	public String getTranslationLanguage1() {
		return translationLanguage1;
	}

	public void setTranslationLanguage1(String translationLanguage1) {
		this.translationLanguage1 = translationLanguage1;
	}

	public String getTranslationUpload1() {
		return translationUpload1;
	}

	public void setTranslationUpload1(String translationUpload1) {
		this.translationUpload1 = translationUpload1;
	}

	public String getTranslationLanguage2() {
		return translationLanguage2;
	}

	public void setTranslationLanguage2(String translationLanguage2) {
		this.translationLanguage2 = translationLanguage2;
	}

	public String getTranslationUpload2() {
		return translationUpload2;
	}

	public void setTranslationUpload2(String translationUpload2) {
		this.translationUpload2 = translationUpload2;
	}

	public String getTranslationLanguage3() {
		return translationLanguage3;
	}

	public void setTranslationLanguage3(String translationLanguage3) {
		this.translationLanguage3 = translationLanguage3;
	}

	public String getTranslationUpload3() {
		return translationUpload3;
	}

	public void setTranslationUpload3(String translationUpload3) {
		this.translationUpload3 = translationUpload3;
	}

	public String getTranslationLanguage4() {
		return translationLanguage4;
	}

	public void setTranslationLanguage4(String translationLanguage4) {
		this.translationLanguage4 = translationLanguage4;
	}

	public String getTranslationUpload4() {
		return translationUpload4;
	}

	public void setTranslationUpload4(String translationUpload4) {
		this.translationUpload4 = translationUpload4;
	}

	public String getTranslationLanguage5() {
		return translationLanguage5;
	}

	public void setTranslationLanguage5(String translationLanguage5) {
		this.translationLanguage5 = translationLanguage5;
	}

	public String getTranslationUpload5() {
		return translationUpload5;
	}

	public void setTranslationUpload5(String translationUpload5) {
		this.translationUpload5 = translationUpload5;
	}

	public String getTranslationPortfolioLink() {
		return translationPortfolioLink;
	}

	public void setTranslationPortfolioLink(String translationPortfolioLink) {
		this.translationPortfolioLink = translationPortfolioLink;
	}

	public String getTranslationPerWordMin() {
		return translationPerWordMin;
	}

	public void setTranslationPerWordMin(String translationPerWordMin) {
		this.translationPerWordMin = translationPerWordMin;
	}

	public String getTranslationPerWordMax() {
		return translationPerWordMax;
	}

	public void setTranslationPerWordMax(String translationPerWordMax) {
		this.translationPerWordMax = translationPerWordMax;
	}

	public String getInterServiceType() {
		return interServiceType;
	}

	public void setInterServiceType(String interServiceType) {
		this.interServiceType = interServiceType;
	}

	public String getIexperience() {
		return iexperience;
	}

	public void setIexperience(String iexperience) {
		this.iexperience = iexperience;
	}

	public String getInterpretationLanguages() {
		return interpretationLanguages;
	}

	public void setInterpretationLanguages(String interpretationLanguages) {
		this.interpretationLanguages = interpretationLanguages;
	}

	public String getInterLanguage1() {
		return interLanguage1;
	}

	public void setInterLanguage1(String interLanguage1) {
		this.interLanguage1 = interLanguage1;
	}

	public String getInterUpload1() {
		return interUpload1;
	}

	public void setInterUpload1(String interUpload1) {
		this.interUpload1 = interUpload1;
	}

	public String getInterLanguage2() {
		return interLanguage2;
	}

	public void setInterLanguage2(String interLanguage2) {
		this.interLanguage2 = interLanguage2;
	}

	public String getInterUpload2() {
		return interUpload2;
	}

	public void setInterUpload2(String interUpload2) {
		this.interUpload2 = interUpload2;
	}

	public String getInterLanguage3() {
		return interLanguage3;
	}

	public void setInterLanguage3(String interLanguage3) {
		this.interLanguage3 = interLanguage3;
	}

	public String getInterUpload3() {
		return interUpload3;
	}

	public void setInterUpload3(String interUpload3) {
		this.interUpload3 = interUpload3;
	}

	public String getInterLanguage4() {
		return interLanguage4;
	}

	public void setInterLanguage4(String interLanguage4) {
		this.interLanguage4 = interLanguage4;
	}

	public String getInterUpload4() {
		return interUpload4;
	}

	public void setInterUpload4(String interUpload4) {
		this.interUpload4 = interUpload4;
	}

	public String getInterLanguage5() {
		return interLanguage5;
	}

	public void setInterLanguage5(String interLanguage5) {
		this.interLanguage5 = interLanguage5;
	}

	public String getInterUpload5() {
		return interUpload5;
	}

	public void setInterUpload5(String interUpload5) {
		this.interUpload5 = interUpload5;
	}

	public String getInterPortfolioLink() {
		return interPortfolioLink;
	}

	public void setInterPortfolioLink(String interPortfolioLink) {
		this.interPortfolioLink = interPortfolioLink;
	}

	public String getInterPerWordMin() {
		return interPerWordMin;
	}

	public void setInterPerWordMin(String interPerWordMin) {
		this.interPerWordMin = interPerWordMin;
	}

	public String getInterPerWordMax() {
		return interPerWordMax;
	}

	public void setInterPerWordMax(String interPerWordMax) {
		this.interPerWordMax = interPerWordMax;
	}

	public String getVcServiceType() {
		return vcServiceType;
	}

	public void setVcServiceType(String vcServiceType) {
		this.vcServiceType = vcServiceType;
	}

	public String getVcExperience() {
		return vcExperience;
	}

	public void setVcExperience(String vcExperience) {
		this.vcExperience = vcExperience;
	}

	public String getVcIndustries() {
		return vcIndustries;
	}

	public void setVcIndustries(String vcIndustries) {
		this.vcIndustries = vcIndustries;
	}

	public String getVcLanguages() {
		return vcLanguages;
	}

	public void setVcLanguages(String vcLanguages) {
		this.vcLanguages = vcLanguages;
	}

	public String getVcLanguage1() {
		return vcLanguage1;
	}

	public void setVcLanguage1(String vcLanguage1) {
		this.vcLanguage1 = vcLanguage1;
	}

	public String getVoiceDesc1() {
		return voiceDesc1;
	}

	public void setVoiceDesc1(String voiceDesc1) {
		this.voiceDesc1 = voiceDesc1;
	}

	public String getVcUpload1() {
		return vcUpload1;
	}

	public void setVcUpload1(String vcUpload1) {
		this.vcUpload1 = vcUpload1;
	}

	public String getVcLanguage2() {
		return vcLanguage2;
	}

	public void setVcLanguage2(String vcLanguage2) {
		this.vcLanguage2 = vcLanguage2;
	}

	public String getVoiceDesc2() {
		return voiceDesc2;
	}

	public void setVoiceDesc2(String voiceDesc2) {
		this.voiceDesc2 = voiceDesc2;
	}

	public String getVcUpload2() {
		return vcUpload2;
	}

	public void setVcUpload2(String vcUpload2) {
		this.vcUpload2 = vcUpload2;
	}

	public String getVcLanguage3() {
		return vcLanguage3;
	}

	public void setVcLanguage3(String vcLanguage3) {
		this.vcLanguage3 = vcLanguage3;
	}

	public String getVoiceDesc3() {
		return voiceDesc3;
	}

	public void setVoiceDesc3(String voiceDesc3) {
		this.voiceDesc3 = voiceDesc3;
	}

	public String getVcUpload3() {
		return vcUpload3;
	}

	public void setVcUpload3(String vcUpload3) {
		this.vcUpload3 = vcUpload3;
	}

	public String getVcLanguage4() {
		return vcLanguage4;
	}

	public void setVcLanguage4(String vcLanguage4) {
		this.vcLanguage4 = vcLanguage4;
	}

	public String getVoiceDesc4() {
		return voiceDesc4;
	}

	public void setVoiceDesc4(String voiceDesc4) {
		this.voiceDesc4 = voiceDesc4;
	}

	public String getVcUpload4() {
		return vcUpload4;
	}

	public void setVcUpload4(String vcUpload4) {
		this.vcUpload4 = vcUpload4;
	}

	public String getVcLanguage5() {
		return vcLanguage5;
	}

	public void setVcLanguage5(String vcLanguage5) {
		this.vcLanguage5 = vcLanguage5;
	}

	public String getVoiceDesc5() {
		return voiceDesc5;
	}

	public void setVoiceDesc5(String voiceDesc5) {
		this.voiceDesc5 = voiceDesc5;
	}

	public String getVcUpload5() {
		return vcUpload5;
	}

	public void setVcUpload5(String vcUpload5) {
		this.vcUpload5 = vcUpload5;
	}

	public String getVcPortfolioLink() {
		return vcPortfolioLink;
	}

	public void setVcPortfolioLink(String vcPortfolioLink) {
		this.vcPortfolioLink = vcPortfolioLink;
	}

	public String getVcPerMinuteMin() {
		return vcPerMinuteMin;
	}

	public void setVcPerMinuteMin(String vcPerMinuteMin) {
		this.vcPerMinuteMin = vcPerMinuteMin;
	}

	public String getVcPerMinuteMax() {
		return vcPerMinuteMax;
	}

	public void setVcPerMinuteMax(String vcPerMinuteMax) {
		this.vcPerMinuteMax = vcPerMinuteMax;
	}

	public String getVcPerHourMin() {
		return vcPerHourMin;
	}

	public void setVcPerHourMin(String vcPerHourMin) {
		this.vcPerHourMin = vcPerHourMin;
	}

	public String getVcPerHourMax() {
		return vcPerHourMax;
	}

	public void setVcPerHourMax(String vcPerHourMax) {
		this.vcPerHourMax = vcPerHourMax;
	}

	public String getVcPerWordMin() {
		return vcPerWordMin;
	}

	public void setVcPerWordMin(String vcPerWordMin) {
		this.vcPerWordMin = vcPerWordMin;
	}

	public String getVcPerWordMax() {
		return vcPerWordMax;
	}

	public void setVcPerWordMax(String vcPerWordMax) {
		this.vcPerWordMax = vcPerWordMax;
	}

	public String getScribingServiceType() {
		return scribingServiceType;
	}

	public void setScribingServiceType(String scribingServiceType) {
		this.scribingServiceType = scribingServiceType;
	}

	public String getScribingExperience() {
		return scribingExperience;
	}

	public void setScribingExperience(String scribingExperience) {
		this.scribingExperience = scribingExperience;
	}

	public String getScribingIndustries() {
		return scribingIndustries;
	}

	public void setScribingIndustries(String scribingIndustries) {
		this.scribingIndustries = scribingIndustries;
	}

	public String getTranscribeLanguages() {
		return transcribeLanguages;
	}

	public void setTranscribeLanguages(String transcribeLanguages) {
		this.transcribeLanguages = transcribeLanguages;
	}

	public String getTranscribeLanguage1() {
		return transcribeLanguage1;
	}

	public void setTranscribeLanguage1(String transcribeLanguage1) {
		this.transcribeLanguage1 = transcribeLanguage1;
	}

	public String getTranscribeUpload1() {
		return transcribeUpload1;
	}

	public void setTranscribeUpload1(String transcribeUpload1) {
		this.transcribeUpload1 = transcribeUpload1;
	}

	public String getTranscribeLanguage2() {
		return transcribeLanguage2;
	}

	public void setTranscribeLanguage2(String transcribeLanguage2) {
		this.transcribeLanguage2 = transcribeLanguage2;
	}

	public String getTranscribeUpload2() {
		return transcribeUpload2;
	}

	public void setTranscribeUpload2(String transcribeUpload2) {
		this.transcribeUpload2 = transcribeUpload2;
	}

	public String getTranscribeLanguage3() {
		return transcribeLanguage3;
	}

	public void setTranscribeLanguage3(String transcribeLanguage3) {
		this.transcribeLanguage3 = transcribeLanguage3;
	}

	public String getTranscribeUpload3() {
		return transcribeUpload3;
	}

	public void setTranscribeUpload3(String transcribeUpload3) {
		this.transcribeUpload3 = transcribeUpload3;
	}

	public String getTranscribeLanguage4() {
		return transcribeLanguage4;
	}

	public void setTranscribeLanguage4(String transcribeLanguage4) {
		this.transcribeLanguage4 = transcribeLanguage4;
	}

	public String getTranscribeUpload4() {
		return transcribeUpload4;
	}

	public void setTranscribeUpload4(String transcribeUpload4) {
		this.transcribeUpload4 = transcribeUpload4;
	}

	public String getScribingPortfolioLink() {
		return scribingPortfolioLink;
	}

	public void setScribingPortfolioLink(String scribingPortfolioLink) {
		this.scribingPortfolioLink = scribingPortfolioLink;
	}

	public String getScribingPerWordMin() {
		return scribingPerWordMin;
	}

	public void setScribingPerWordMin(String scribingPerWordMin) {
		this.scribingPerWordMin = scribingPerWordMin;
	}

	public String getScribingPerWordMax() {
		return scribingPerWordMax;
	}

	public void setScribingPerWordMax(String scribingPerWordMax) {
		this.scribingPerWordMax = scribingPerWordMax;
	}

	@Override
	public String toString() {
		return "FreelancerSkillDetailDTO [translationServiceType=" + translationServiceType + ", translationExperience="
				+ translationExperience + ", translationIndustries=" + translationIndustries + ", translationLanguages="
				+ translationLanguages + ", translationLanguage1=" + translationLanguage1 + ", translationUpload1="
				+ translationUpload1 + ", translationLanguage2=" + translationLanguage2 + ", translationUpload2="
				+ translationUpload2 + ", translationLanguage3=" + translationLanguage3 + ", translationUpload3="
				+ translationUpload3 + ", translationLanguage4=" + translationLanguage4 + ", translationUpload4="
				+ translationUpload4 + ", translationLanguage5=" + translationLanguage5 + ", translationUpload5="
				+ translationUpload5 + ", translationPortfolioLink=" + translationPortfolioLink
				+ ", translationPerWordMin=" + translationPerWordMin + ", translationPerWordMax="
				+ translationPerWordMax + ", interServiceType=" + interServiceType + ", iexperience=" + iexperience
				+ ", interpretationLanguages=" + interpretationLanguages + ", interLanguage1=" + interLanguage1
				+ ", interUpload1=" + interUpload1 + ", interLanguage2=" + interLanguage2 + ", interUpload2="
				+ interUpload2 + ", interLanguage3=" + interLanguage3 + ", interUpload3=" + interUpload3
				+ ", interLanguage4=" + interLanguage4 + ", interUpload4=" + interUpload4 + ", interLanguage5="
				+ interLanguage5 + ", interUpload5=" + interUpload5 + ", interPortfolioLink=" + interPortfolioLink
				+ ", interPerWordMin=" + interPerWordMin + ", interPerWordMax=" + interPerWordMax + ", vcServiceType="
				+ vcServiceType + ", vcExperience=" + vcExperience + ", vcIndustries=" + vcIndustries + ", vcLanguages="
				+ vcLanguages + ", vcLanguage1=" + vcLanguage1 + ", voiceDesc1=" + voiceDesc1 + ", vcUpload1="
				+ vcUpload1 + ", vcLanguage2=" + vcLanguage2 + ", voiceDesc2=" + voiceDesc2 + ", vcUpload2=" + vcUpload2
				+ ", vcLanguage3=" + vcLanguage3 + ", voiceDesc3=" + voiceDesc3 + ", vcUpload3=" + vcUpload3
				+ ", vcLanguage4=" + vcLanguage4 + ", voiceDesc4=" + voiceDesc4 + ", vcUpload4=" + vcUpload4
				+ ", vcLanguage5=" + vcLanguage5 + ", voiceDesc5=" + voiceDesc5 + ", vcUpload5=" + vcUpload5
				+ ", vcPortfolioLink=" + vcPortfolioLink + ", vcPerMinuteMin=" + vcPerMinuteMin + ", vcPerMinuteMax="
				+ vcPerMinuteMax + ", vcPerHourMin=" + vcPerHourMin + ", vcPerHourMax=" + vcPerHourMax
				+ ", vcPerWordMin=" + vcPerWordMin + ", vcPerWordMax=" + vcPerWordMax + ", scribingServiceType="
				+ scribingServiceType + ", scribingExperience=" + scribingExperience + ", scribingIndustries="
				+ scribingIndustries + ", transcribeLanguages=" + transcribeLanguages + ", transcribeLanguage1="
				+ transcribeLanguage1 + ", transcribeUpload1=" + transcribeUpload1 + ", transcribeLanguage2="
				+ transcribeLanguage2 + ", transcribeUpload2=" + transcribeUpload2 + ", transcribeLanguage3="
				+ transcribeLanguage3 + ", transcribeUpload3=" + transcribeUpload3 + ", transcribeLanguage4="
				+ transcribeLanguage4 + ", transcribeUpload4=" + transcribeUpload4 + ", scribingPortfolioLink="
				+ scribingPortfolioLink + ", scribingPerWordMin=" + scribingPerWordMin + ", scribingPerWordMax="
				+ scribingPerWordMax + "]";
	}
}
